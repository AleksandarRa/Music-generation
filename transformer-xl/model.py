...
from config_music import n_units_sound
...
class LSTM_layer(tf.keras.layers.Layer):
    ...
    def __init__(self, d_model, n_units, dropout_rate, gating_type=None):
...
class LSTM_network(tf.keras.Model):
    ...
    def __init__(self, d_sound, d_delta, n_units_sound, n_units_delta, n_units_combined,
        ...
        assert d_sound % n_units_sound == 0
        assert d_delta % n_units_delta == 0
        assert (d_sound + d_delta) % n_units_combined== 0
        assert 0.0 <= dropout_rate < 1.0

        self.d_sound = d_sound
        self.d_delta = d_delta
        self.d_combined = d_sound + d_delta
        self.n_units_sound = n_units_sound
        self.n_units_delta = n_units_delta
        self.n_units_combined = n_units_combined
    ...
    def build_from_config(config, checkpoint_path=None, optimizer_path=None):
        ...
        model = LSTM_network(d_sound=config.d_sound, d_delta=config.d_delta,
                                  n_units_sound=config.n_units_sound, n_units_delta=config.n_units_delta,
                                  n_units_combined=config.n_heads_combined, n_layers_sound=config.n_layers_sound,
